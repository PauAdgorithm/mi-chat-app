...
interface LoginProps {
  // AÑADIDO: Ahora aceptamos el parámetro "remember"
  onLogin: (username: string, role: string, remember: boolean) => void;
  socket: any;
}
...
export function Login({ onLogin, socket }: LoginProps) {
  ...
  // ESTADO NUEVO: Recordarme
  const [rememberMe, setRememberMe] = useState(true); // Por defecto marcado, es más cómodo
  ...
  useEffect(() => {
    if (socket) {
        ...
        // Pasamos "rememberMe" al callback de éxito
        socket.on('login_success', (user: any) => onLogin(user.username, user.role, rememberMe));
        socket.on('login_error', (msg: string) => setError(msg));
    }
  }, [socket, rememberMe]); // Añadimos rememberMe a dependencias para que el socket use el valor actual
      ...
      {!selectedAgent && (
          <div className="w-full space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
              {loading ? <RefreshCw className="w-6 h-6 animate-spin mx-auto text-slate-400" /> : 
               agents.map((agent) => (
                  <button key={agent.id} onClick={() => { setSelectedAgent(agent); setError(''); }} className="w-full flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-500 hover:shadow-md transition-all group text-left">
                      <div className="flex items-center gap-3">
                          <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white font-bold ${agent.role === 'Admin' ? 'bg-purple-600' : 'bg-blue-500'}`}>{agent.name.charAt(0).toUpperCase()}</div>
                          <div><h3 className="font-bold text-slate-700">{agent.name}</h3><span className="text-xs text-slate-400">{agent.role}</span></div>
                      </div>
      ...
      {selectedAgent && (
          <form onSubmit={handleLogin} className="w-full bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
              {selectedAgent.hasPassword ? (
                  <div className="mb-4"><label className="text-xs font-bold text-slate-500 uppercase">Contraseña</label><div className="relative mt-1"><Lock className="w-4 h-4 absolute left-3 top-3.5 text-slate-400" /><input type="password" autoFocus value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full p-3 pl-10 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-blue-500" placeholder="••••••" /></div></div>
              ) : <div className="mb-6 text-center p-4 bg-green-50 rounded-xl border border-green-100 text-green-700 text-sm font-medium">Acceso libre</div>}
              ...
              >
                  {rememberMe ? <CheckSquare className="w-5 h-5 text-blue-600" /> : <Square className="w-5 h-5 text-slate-400" />}
                  <span className={`text-sm ${rememberMe ? 'text-slate-700 font-medium' : 'text-slate-400'}`}>Mantener sesión iniciada</span>
              </div>
