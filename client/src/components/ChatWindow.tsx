...
interface ChatWindowProps {
  ...
  onBack: () => void; // NUEVO: Función para volver atrás en móvil
}
...
// ... (El componente CustomAudioPlayer SE MANTIENE IGUAL QUE ANTES, COPIALO DE TU CÓDIGO ACTUAL O DEL ANTERIOR) ...
// Para ahorrar espacio aquí, asumo que CustomAudioPlayer ya lo tienes. 
// Si necesitas que lo pegue otra vez, pídemelo.
const CustomAudioPlayer = ({ src, isMe }: { src: string, isMe: boolean }) => {
  ...
  useEffect(() => { fetch(src).then(r => r.blob()).then(blob => { setAudioUrl(URL.createObjectURL(blob)); setIsReady(true); }).catch(e => console.error("Error audio", e)); }, [src]);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative" onClick={() => setShowEmojiPicker(false)}>
      ...
      <div className="bg-white border-b border-gray-200 p-3 flex flex-wrap gap-3 items-center shadow-sm z-10" onClick={(e) => e.stopPropagation()}>
        ...
        <button 
            onClick={onBack}
            className="md:hidden p-2 rounded-full text-slate-500 hover:bg-slate-100 mr-1"
        >
        ...
        <div className="flex items-center gap-2 bg-purple-50 px-2 rounded-md border border-purple-200">
            ...
            <select 
                className="text-xs bg-transparent border-none rounded-md py-1.5 pr-6 text-purple-700 focus:ring-0 cursor-pointer font-bold uppercase tracking-wide" 
                value={department} 
        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex flex-col max-w-[85%] md:max-w-[75%]`}> {/* Aumenté ancho en móvil */}
                ...
                <div className={`p-3 rounded-xl shadow-sm text-sm relative text-slate-800 ${isMe ? 'bg-green-100 rounded-tr-none' : 'bg-white rounded-tl-none border border-slate-100'}`}>
                    {m.type === 'image' && m.mediaId ? (
                        <div className="mb-1 group relative"><img src={`${API_URL}/api/media/${m.mediaId}`} alt="Imagen" className="rounded-lg max-w-full md:max-w-[280px] h-auto object-contain" onClick={(e) => { e.stopPropagation(); setSelectedImage(`${API_URL}/api/media/${m.mediaId}`); }} /></div>
