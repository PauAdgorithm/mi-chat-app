...
interface SettingsProps {
  onBack: () => void;
  socket: any;
  currentUserRole: string; // Necesitamos saber quién eres
}
...
export function Settings({ onBack, socket, currentUserRole }: SettingsProps) {
  ...
  const closeModal = () => {
      setModalType('none');
      setAdminPass(''); setFormName(''); setFormPass(''); setError(''); setSelectedItem(null);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      switch (modalType) {
          case 'create_agent':
              socket.emit('create_agent', { newAgent: { name: formName, role: formRole, password: formPass }, adminPassword: adminPass });
              break;
          case 'edit_agent':
              ...
              socket.emit('update_agent', { agentId: selectedItem.id, updates, adminPassword: adminPass });
              break;
          case 'delete_agent':
              socket.emit('delete_agent', { agentId: selectedItem.id, adminPassword: adminPass });
              break;
          case 'add_config':
              socket.emit('add_config', { name: formName, type: formType, adminPassword: adminPass });
              break;
          case 'edit_config':
              socket.emit('update_config', { id: selectedItem.id, name: formName, adminPassword: adminPass });
              break;
          case 'delete_config':
              socket.emit('delete_config', { id: selectedItem.id, adminPassword: adminPass });
              break;
      }
  };
    ...
    <div className="fixed inset-0 bg-slate-50 z-50 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
          <div className="w-64 bg-white border-r border-gray-200 p-4 space-y-2">
              <button onClick={() => setActiveTab('team')} className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'team' ? 'bg-blue-50 text-blue-600' : 'text-slate-500 hover:bg-slate-50'}`}><User className="w-5 h-5" /> Gestión de Equipo</button>
              <button onClick={() => setActiveTab('config')} className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'config' ? 'bg-purple-50 text-purple-600' : 'text-slate-500 hover:bg-slate-50'}`}><LayoutList className="w-5 h-5" /> Dptos. y Estados</button>
          </div>
                          ...
                          {agents.map(agent => (
                              <div key={agent.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 group">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div><p className="font-bold text-slate-700 text-sm">{agent.name}</p><p className="text-xs text-slate-400">{agent.role}</p></div>
                                  </div>
              ...
              {activeTab === 'config' && (
                  currentUserRole !== 'Admin' ? (
                      <div className="flex flex-col items-center justify-center h-full text-slate-400">
                          <ShieldAlert className="w-12 h-12 mb-2 text-red-300" />
                          <h3 className="text-lg font-bold text-slate-600">Acceso Restringido</h3>
                          <p>Solo los administradores pueden editar la configuración global.</p>
                      </div>
                  ) : (
          ...
          <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 animate-in fade-in">
              <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                      {/* Inputs Nombre/Rol (Solo para crear/editar agentes) */}
                      {(modalType === 'create_agent' || modalType === 'edit_agent') && (
                          <>
                            ...
                            <input type="password" value={formPass} onChange={e => setFormPass(e.target.value)} placeholder={modalType === 'edit_agent' ? "Nueva contraseña (Opcional)" : "Contraseña"} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg" />
                          </>
                      )}

                      {/* Input Nombre Config (Dept/Status) */}
                      ...
                      {/* CONTRASEÑA ADMIN (OBLIGATORIA SIEMPRE) */}
                      <div className="border-t border-slate-100 pt-4">
                          <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Contraseña de Administrador</label>
                          <input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="Requerida para confirmar" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:border-blue-500 outline-none" required />
                      </div>
