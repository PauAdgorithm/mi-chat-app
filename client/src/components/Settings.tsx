...
interface SettingsProps {
  onBack: () => void;
  socket: any;
  currentUserRole: string; // IMPORTANTE: Esta prop debe coincidir con App.tsx
}
      ...
      return (
        <div className="fixed inset-0 bg-slate-50 z-50 flex flex-col">
             ...
             <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center animate-in fade-in">
                  <div className="bg-red-50 p-6 rounded-full mb-4 border border-red-100"><ShieldAlert className="w-16 h-16 text-red-400" /></div>
                  <h3 className="text-2xl font-bold text-slate-700 mb-2">Acceso Restringido</h3>
                  <p className="text-slate-500 max-w-md">Solo los administradores tienen permiso.</p>
             </div>
        </div>
      );
  }
    ...
    <div className="fixed inset-0 bg-slate-50 z-50 flex flex-col h-full w-full">
      <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between shadow-sm flex-shrink-0">
          <div className="flex items-center gap-3">
              <button onClick={handleBack} className="p-2 hover:bg-slate-100 rounded-full transition"><ArrowLeft className="w-6 h-6 text-slate-600" /></button>
              <h1 className="text-lg md:text-xl font-bold text-slate-800 truncate">{!showMobileMenu ? (activeTab === 'team' ? 'Gestión Equipo' : 'Dptos. y Estados') : 'Configuración'}</h1>
          </div>
      ...
      {modalType !== 'none' && (
          <div className="fixed inset-0 bg-black/50 z-[60] flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in backdrop-blur-sm">
              <div className="bg-white w-full md:max-w-md rounded-t-2xl md:rounded-2xl shadow-2xl p-6 animate-in slide-in-from-bottom-10 md:zoom-in-95 max-h-[90vh] overflow-y-auto">
                  <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">{modalType.includes('create') ? 'Crear' : modalType.includes('edit') ? 'Editar' : 'Eliminar'}</h3><button onClick={closeModal} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200"><X className="w-5 h-5 text-slate-600"/></button></div>
