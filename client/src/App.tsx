...
function App() {
  ...
  useEffect(() => {
    // 1. Recuperar sesión (LocalStorage o SessionStorage)
    let savedUser = localStorage.getItem('chatgorithm_user');
    if (!savedUser) savedUser = sessionStorage.getItem('chatgorithm_user');
  ...
  const handleLogin = (username: string, role: string, password: string, remember: boolean) => {
    const u = { username, role, password }; // Guardamos pass en memoria para acciones de admin
    setUser(u);
