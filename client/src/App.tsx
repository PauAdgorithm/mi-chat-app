...
function App() {
  ...
  // CORRECCIÓN AQUÍ: prefs es opcional y tiene valor por defecto {}
  ...
  const handleLogout = () => { localStorage.removeItem('chatgorithm_user'); setUser(null); setSelectedContact(null); };

  if (!user) return <Login onLogin={handleLogin} socket={socket} />;
        ...
        <div className="flex w-full h-full max-w-[1800px] mx-auto bg-white shadow-2xl overflow-hidden md:h-screen border-x border-gray-200">
          ...
          <div className={`w-full md:w-80 flex-shrink-0 flex-col border-r border-gray-100 bg-slate-50/50 ${selectedContact ? 'hidden md:flex' : 'flex'}`}>
            ...
            <div className="p-3 border-t border-slate-200 bg-white flex gap-2">
                <button onClick={() => setView('settings')} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg"><SettingsIcon className="w-5 h-5" /></button>
