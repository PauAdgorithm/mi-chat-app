...
console.log("🚀 [BOOT] Arrancando servidor MAESTRO (Full Features)...");
dotenv.config();
...
async function sendWhatsAppText(to: string, body: string, originPhoneId: string) {
    ...
    try {
        ...
        await handleContactUpdate(to, `🤖 Bot: ${body}`, undefined, originPhoneId);
...
app.get('/api/templates', async (req, res) => {
    ...
    try {
        ...
        res.json(r.map(x => ({ 
            ...
            status: x.get('Status'), 
            body: x.get('Body'), 
            variableMapping: x.get('VariableMapping') ? JSON.parse(x.get('VariableMapping') as string) : {} 
        })));
    ...
    try {
        ...
        const createdRecords = await base(TABLE_TEMPLATES).create([{
            fields: {
                ...
                "Body": body, "Footer": footer, "Status": status, "MetaId": metaId,
                "VariableMapping": JSON.stringify(variableExamples || {})
            }
        }]);

        res.json({ success: true, template: { id: createdRecords[0].id } });
...
app.post('/api/send-template', async (req, res) => {
    ...
    try {
        ...
        await axios.post(
            ...
            { messaging_product: "whatsapp", to: cleanNumber(phone), type: "template", template: { name: templateName, language: { code: language }, components: [{ type: "body", parameters }] } },
            { headers: { Authorization: `Bearer ${token}` } }
        );
...
app.get('/api/analytics', async (req, res) => {
    ...
    try {
        ...
        const activityData = last7Days.map(date => {
            ...
            return { date, label: new Date(date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }), count };
        });

        const agentStats: Record<string, any> = {}; 
        messages.forEach(m => {
            const s = (m.get('sender') as string) || "";
            const isPhone = /^\d+$/.test(s.replace(/\D/g, ''));
            if (!isPhone && s.toLowerCase() !== 'sistema' && s.trim() !== '') {
                if (!agentStats[s]) agentStats[s] = { msgs: 0, uniqueChats: new Set() };
                agentStats[s].msgs += 1;
                if (m.get('recipient')) agentStats[s].uniqueChats.add(m.get('recipient'));
            }
        });
...
app.post('/api/upload', upload.single('file'), async (req: any, res: any) => { 
    try { 
        ...
        await handleContactUpdate(targetPhone, `Tú: 📎 Archivo`, undefined, fromId); 
        res.json({ success: true }); 
...
async function handleContactUpdate(phone: string, text: string, name: string = "Cliente", originId: string = "unknown") {
  ...
  try {
    ...
    // Fix Huérfanos: Si no existe, buscar si hay uno sin asignar y adoptarlo
    if (r.length === 0) {
...
io.on('connection', (socket) => {
  // Sockets Config, Quick Replies, Agents... (Se mantienen)
  ...
  socket.on('request_agents', async () => { if (base) { const r = await base('Agents').select().all(); socket.emit('agents_list', r.map(x => ({ id: x.id, name: x.get('name'), role: x.get('role'), hasPassword: !!x.get('password'), preferences: x.get('Preferences') }))); } });
  ...
  socket.on('create_agent', async (d) => { if (!base) return; await base('Agents').create([{ fields: { "name": d.newAgent.name, "role": d.newAgent.role, "password": d.newAgent.password || "" } }]); const r = await base('Agents').select().all(); io.emit('agents_list', r.map(x => ({ id: x.id, name: x.get('name'), role: x.get('role'), hasPassword: !!x.get('password'), preferences: x.get('Preferences') }))); socket.emit('action_success', 'Creado'); });
  socket.on('delete_agent', async (d) => { if (!base) return; await base('Agents').destroy([d.agentId]); const r = await base('Agents').select().all(); io.emit('agents_list', r.map(x => ({ id: x.id, name: x.get('name'), role: x.get('role'), hasPassword: !!x.get('password'), preferences: x.get('Preferences') }))); socket.emit('action_success', 'Eliminado'); });
  socket.on('update_agent', async (d) => { if (!base) return; const f: any = { "name": d.updates.name, "role": d.updates.role }; if (d.updates.password !== undefined) f["password"] = d.updates.password; if (d.updates.preferences !== undefined) f["Preferences"] = JSON.stringify(d.updates.preferences); await base('Agents').update([{ id: d.agentId, fields: f }]); const r = await base('Agents').select().all(); io.emit('agents_list', r.map(x => ({ id: x.id, name: x.get('name'), role: x.get('role'), hasPassword: !!x.get('password'), preferences: x.get('Preferences') }))); socket.emit('action_success', 'Actualizado'); });
