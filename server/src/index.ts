...
console.log("🚀 [BOOT] Arrancando servidor MAESTRO (Strict Verification Mode)...");
dotenv.config();
...
// ==========================================
//  NUEVAS RUTAS: GESTIÓN DE CONTACTOS (ESTRICTA)
// ==========================================
...
app.post('/api/contacts', async (req, res) => {
    ...
    try {
        ...
        if (token && originId !== 'default') {
            try {
                console.log(`🔍 Verificando existencia de ${cleanP} en WhatsApp (ID: ${originId})...`);
                ...
                if (status !== 'valid') {
                    // SI NO ES VÁLIDO, PARAMOS AQUÍ.
                    return res.status(400).json({ error: "El número NO tiene WhatsApp activo. No se puede guardar." });
                }
                console.log(`✅ Número válido: ${cleanP}`);
            } catch (metaError: any) {
                console.error("Error crítico verificando en Meta:", metaError.response?.data || metaError.message);
                
                // Si el error es de configuración (ID incorrecto), avisamos claramente
                if (metaError.response?.data?.error?.code === 100) {
                     return res.status(500).json({ 
                         error: "Error de Configuración del Servidor: El ID de WhatsApp (PHONE_ID) es incorrecto. Verifica que no sea el WABA ID." 
                     });
                }

                // Si es otro error (ej: número mal formado), paramos
                return res.status(400).json({ error: "Error al verificar con WhatsApp. Revisa el número y el prefijo." });
            }
        }
        ...
        res.status(500).json({ error: e.message || "Error creando contacto" });
    }
});
