...
console.log("🚀 [BOOT] Arrancando servidor FINAL con Agenda y Fix IDs...");
dotenv.config();
...
async function bookAppointment(appointmentId: string, clientPhone: string, clientName: string) {
    ...
    // --- FIX CRÍTICO: LIMPIEZA DE ID ---
    // Buscamos un patrón que empiece por 'rec' y tenga caracteres alfanuméricos
    ...
    // Si el ID es muy corto, es basura (ej: "rec4")
...
app.post('/api/appointments', async (req, res) => {
    if (!base) return res.status(500).json({ error: "DB" });
    try {
        await base('Appointments').create([{ fields: { "Date": req.body.date, "Status": req.body.status || 'Available' } }]);
...
app.post('/api/appointments/generate', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true });
    } catch(e: any) { res.status(400).json({error: "Error"}); }
});
...
app.put('/api/appointments/:id', async (req, res) => {
    ...
    try {
        const f:any={}; 
        if(req.body.date)f["Date"]=req.body.date; 
...
app.delete('/api/appointments/:id', async (req, res) => {
    if (!base) return res.status(500).json({ error: "DB" });
    try { await base('Appointments').destroy([req.params.id]); res.json({ success: true }); } catch(e) { res.status(400).json({error: "Error"}); }
});
