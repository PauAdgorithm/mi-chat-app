...
console.log("🚀 [BOOT] Arrancando servidor con IA Laura v11 (Anti-Alucinaciones)...");
dotenv.config();
...
// RUTA "PING"
app.get('/', (req, res) => {
  res.send('🤖 Servidor Chatgorithm (Laura v11) funcionando correctamente 🚀');
});
...
async function bookAppointment(appointmentId: string, clientPhone: string, clientName: string) {
    ...
    // LIMPIEZA EXTRAMA DEL ID
    let cleanId = appointmentId.trim().replace(/['"]/g, '');
    // Quitamos prefijos comunes de alucinación
    ...
    console.log(`📅 RESERVA: ID recibido "${appointmentId}" -> Limpio "${cleanId}"`);
    ...
    try {
        ...
        try {
            ...
            return "❌ ERROR: El ID que has usado NO EXISTE en la base de datos. Probablemente te lo has inventado. Vuelve a usar 'get_available_appointments' y usa un ID real de la lista.";
        }
