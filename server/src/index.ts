...
app.post('/api/send-template', async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({ error: "Error envío" });
    }
});
...
app.post('/api/upload', upload.single('file'), async (req: any, res: any) => {
    try {
        ...
        await saveAndEmitMessage({
            text: textLog,
            sender: senderName || "Agente",
            recipient: cleanNumber(targetPhone),
            timestamp: new Date().toISOString(),
            type: saveType,
            mediaId
        });
...
io.on('connection', (socket) => {
    ...
    if (waToken && waPhoneId) {
      try {
        if (msg.type !== 'note') {
          ...
          console.log("📝 Nota interna guardada");
        }
        ...
        const previewText = msg.type === 'note'
          ? `📝 Nota: ${msg.text}`
  ...
  socket.on('stop_ai_manual', (d) => {
    activeAiChats.delete(cleanNumber(d.phone));
    io.emit('ai_active_change', { phone: cleanNumber(d.phone), active: false });
  });

  socket.on('register_presence', (u: string) => {
    if (u) {
