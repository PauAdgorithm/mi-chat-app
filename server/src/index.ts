...
console.log("🚀 [BOOT] Arrancando servidor MAESTRO (Verificación de Números)...");
dotenv.config();
...
// ==========================================
//  NUEVAS RUTAS: GESTIÓN DE CONTACTOS (CON VERIFICACIÓN)
// ==========================================

// 1. Crear Contacto Manual (CON VERIFICACIÓN DE WHATSAPP)
app.post('/api/contacts', async (req, res) => {
    if (!base) return res.status(500).json({ error: "DB no conectada" });
    try {
        const { phone, name, originPhoneId, email, address } = req.body;
        const cleanP = cleanNumber(phone);
        const originId = originPhoneId || waPhoneId || "default";
        const token = getToken(originId);

        // 1. VALIDACIÓN PREVIA EN META (CHECK WHATSAPP EXISTENCE)
        if (token && originId !== 'default') {
            try {
                console.log(`🔍 Verificando existencia de ${cleanP} en WhatsApp...`);
                const checkRes = await axios.post(
                    `https://graph.facebook.com/v17.0/${originId}/contacts`,
                    { blocking: "wait", contacts: [cleanP], force_check: true },
                    { headers: { Authorization: `Bearer ${token}` } }
                );

                const status = checkRes.data.contacts?.[0]?.status;
                if (status !== 'valid') {
                    console.warn(`❌ Número inválido: ${cleanP}`);
                    return res.status(400).json({ error: "El número NO tiene una cuenta de WhatsApp activa. No recibirá mensajes." });
                }
                console.log(`✅ Número válido: ${cleanP}`);
            } catch (metaError: any) {
                console.error("Error verificando en Meta (se procederá a guardar con advertencia):", metaError.response?.data || metaError.message);
                // Opcional: Podrías decidir bloquearlo si falla la verificación, o dejar pasar si es error de API.
                // Dejamos pasar pero en el log queda constancia.
            }
        }

        // 2. Verificamos si ya existe en Airtable
...
//  RUTAS API
// ==========================================
app.get('/api/accounts', (req, res) => { const accounts = Object.keys(BUSINESS_ACCOUNTS).map(id => ({ id, name: `Línea ${id.slice(-4)}` })); res.json(accounts); });
...
app.post('/api/appointments', async (req, res) => { if (!base) return res.status(500).json({}); await base('Appointments').create([{ fields: { "Date": req.body.date, "Status": "Available" } }]); res.json({success:true}); });
...
app.post('/api/appointments/generate', async (req, res) => { if (!base) return res.status(500).json({}); try { const { days, startTime, endTime, duration } = req.body; const newSlots = []; const endD = new Date(); endD.setDate(endD.getDate()+30); for (let d = new Date(); d <= endD; d.setDate(d.getDate()+1)) { if (days.includes(d.getDay())) { const s = new Date(d), e = new Date(d); const [sh, sm] = startTime.split(':'), [eh, em] = endTime.split(':'); s.setHours(+sh, +sm, 0, 0); e.setHours(+eh, +em, 0, 0); while (s < e) { newSlots.push({ fields: { "Date": s.toISOString(), "Status": "Available" } }); s.setMinutes(s.getMinutes()+duration); } } } while(newSlots.length>0) await base('Appointments').create(newSlots.splice(0,10)); res.json({ success: true }); } catch(e: any) { res.status(400).json({error: "Error"}); } });
...
app.delete('/api/delete-template/:id', async (req, res) => { if(!base) return res.status(500).json({}); try { await base(TABLE_TEMPLATES).destroy([req.params.id]); res.json({ success: true }); } catch (error: any) { res.status(500).json({ error: "Error" }); } });
app.post('/api/send-template', async (req, res) => { const { templateName, language, phone, variables, senderName, originPhoneId } = req.body; const token = getToken(originPhoneId || waPhoneId || "default"); const fromId = originPhoneId || waPhoneId; if (!token) return res.status(500).json({ error: "Credenciales" }); try { const parameters = variables.map((val: string) => ({ type: "text", text: val })); await axios.post(`https://graph.facebook.com/v17.0/${fromId}/messages`, { messaging_product: "whatsapp", to: cleanNumber(phone), type: "template", template: { name: templateName, language: { code: language }, components: [{ type: "body", parameters }] } }, { headers: { Authorization: `Bearer ${token}` } }); await saveAndEmitMessage({ text: `📝 [Plantilla] ${templateName}`, sender: senderName || "Agente", recipient: cleanNumber(phone), timestamp: new Date().toISOString(), type: "template", origin_phone_id: fromId }); res.json({ success: true }); } catch (e: any) { res.status(400).json({ error: "Error envío" }); } });

app.get('/api/analytics', async (req, res) => { if(!base) return res.status(500).json({}); try { const contacts = await base('Contacts').select().all(); const messages = await base('Messages').select().all(); const totalContacts = contacts.length; const totalMessages = messages.length; const newLeads = contacts.filter(c => c.get('status') === 'Nuevo').length; const last7Days = [...Array(7)].map((_, i) => { const d = new Date(); d.setDate(d.getDate() - i); return d.toISOString().split('T')[0]; }).reverse(); const activityData = last7Days.map(date => { const count = messages.filter(m => { const mDate = (m.get('timestamp') as string || "").split('T')[0]; return mDate === date; }).length; return { date, label: new Date(date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }), count }; }); const agentStats: Record<string, any> = {}; messages.forEach(m => { const s = (m.get('sender') as string) || ""; const r = (m.get('recipient') as string) || ""; const isPhone = /^\d+$/.test(s.replace(/\D/g, '')); if (!isPhone && s.toLowerCase() !== 'sistema' && s.trim() !== '') { if (!agentStats[s]) agentStats[s] = { msgs: 0, uniqueChats: new Set() }; agentStats[s].msgs += 1; if (r) agentStats[s].uniqueChats.add(r); } }); const agentPerformance = Object.entries(agentStats).map(([name, data]) => ({ name, msgCount: data.msgs, chatCount: data.uniqueChats.size })).sort((a, b) => b.msgCount - a.msgCount).slice(0, 5); const statusMap: Record<string, number> = {}; contacts.forEach(c => { const s = (c.get('status') as string) || 'Otros'; statusMap[s] = (statusMap[s] || 0) + 1; }); const statusDistribution = Object.entries(statusMap).map(([name, count]) => ({ name, count })); res.json({ kpis: { totalContacts, totalMessages, newLeads }, activity: activityData, agents: agentPerformance, statuses: statusDistribution }); } catch (e) { res.status(500).json({ error: "Error" }); } });
...
async function handleContactUpdate(phone: string, text: string, name: string = "Cliente", originId: string = "unknown") {
  ...
  try {
    ...
    if (r.length > 0) { await base('Contacts').update([{ id: r[0].id, fields: { "last_message": text, "last_message_time": new Date().toISOString() } }]); return r[0]; } 
    else { const n = await base('Contacts').create([{ fields: { "phone": clean, "name": name, "status": "Nuevo", "last_message": text, "last_message_time": new Date().toISOString(), "origin_phone_id": originId } }]); io.emit('contact_updated_notification'); return n[0]; }
